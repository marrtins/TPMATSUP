...
%asi se define una funcion void, que no recibe parametros
function prueba() 
...
%el valor de 'position' en este caso es [10 400 150 20] que significa [posicionEnX posicionEnY largoDelElemento altoDelElemento]
b1 = uicontrol(h, 'style', 'pushbutton', 'string', 'esto es un boton', 'position', [10 400 150 20]);

%ejemplo de mostrar un texto
b2 = uicontrol(h, 'style', 'text', 'string', 'esto es un texto a mostrar', 'position', [10 370 150 20]);

%ejemplo de campo de texto
b3 = uicontrol(h, 'style', 'edit', 'position', [10 340 150 20]);

%el boton apretable que cree anteriormente no hace nada. para que cuando lo aprete, genere alguna funcionalidad
%tengo que usar la propiedad callback (lo que hace es llamar a la/s funcion/es una vez que ocurre un evento, en este caso apretar el botón), y decirle que funciones se van a ejecutar
%delete(gcf) cierra la ventana actual
b4 = uicontrol(h, 'style', 'pushbutton', 'string', 'boton con funcionalidad', 'position', [10 300 150 20], 'callback', 'delete(gcf)')

%La funcion siempre termina con un endfunction
endfunction

%Si quieren pueden ejecutar este archivo para ver como muestra todos los elementos